###load all assests that will be used for animation of the player
import pygame
...
for i in range(NUM_ATTACK_FRAMES):
    # loop through range of ammount of frames to get each individual png
    attack_path = f'building blocks/warrior_movements/0{i}_Warrior_Attack1.png'

    try:
        #load the image and make its background transparent
        frame_attack = pygame.image.load(attack_path).convert_alpha()
        scaled_attack_frame = pygame.transform.scale(frame_attack, (TARGET_FRAME_WIDTH, TARGET_FRAME_HEIGHT) )
        #append the individual pngs to the list to be looped through later
        ATTACK_FRAMES.append(scaled_attack_frame)
    except pygame.error as e:
        print(f"failed to load image")
        print(f"pygame error")
